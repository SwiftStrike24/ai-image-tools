...
import { convertHeicToJpeg, createObjectURL, revokeObjectURL } from "@/utils/imageUtils"
  ...
  const handleFileChange = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        const objectUrl = createObjectURL(convertedFile);
  ...
  const handleUpscale = useCallback(async () => {
    ...
    try {
      ...
      const base64Image = await new Promise<string>((resolve, reject) => {
        ...
        reader.onerror = (e) => reject(new Error("Failed to read the image file."));
